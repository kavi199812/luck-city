<?php
/*
 * Production vs Waste Report View
 * Displays pre-made food items produced vs wasted on a given date.
 * Columns: Product Name | Production Qty | Waste Qty (sorted by waste qty descending by default).
 */
?>
...
<section class="main-content-wrapper">
    ...
    <section class="content-header">
        <h3 class="top-left-header text-left"><?php echo lang('production_vs_waste_report'); ?></h3>
        <input type="hidden" class="datatable_name" data-title="<?php echo lang('production_vs_waste_report'); ?>" data-id_name="datatable">
    </section>
    ...
    <div class="box-wrapper">
        <!-- Filter Form -->
        <div class="row mb-3">
            <?php echo form_open(base_url() . 'Report/productionVsWasteReport'); ?>

            <!-- Single Date Filter -->
            <div class="col-sm-12 col-md-4 col-lg-2 mb-3">
                <div class="form-group">
                    <input tabindex="1" type="text" id="production_vs_waste_date" name="date" readonly
                ...
                <table id="datatable" class="table table-striped">
                    <thead>
                        <tr>
                            ...
                            <th><?php echo lang('production_qty'); ?></th>
                            <th><?php echo lang('waste_qty'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        $row_num = 0;
                        if (isset($productionVsWasteReport) && !empty($productionVsWasteReport)):
                            foreach ($productionVsWasteReport as $row):
                                $row_num++;
                                $prod_display = trim(getAmtPCustom($row->production_qty) . ' ' . escape_output($row->unit_name));
                                $waste_display = trim(getAmtPCustom($row->waste_qty) . ' ' . escape_output($row->unit_name));
                        ?>
                            <tr>
                                <td class="ir_txt_center"><?php echo $row_num; ?></td>
                                <td><?php echo escape_output($row->product_name); ?></td>
                                <td><?php echo $prod_display; ?></td>
                                <td><?php echo $waste_display; ?></td>
                            </tr>
                        <?php
                            endforeach;
                        else:
                    ...
                    <?php if (isset($productionVsWasteReport) && !empty($productionVsWasteReport)): ?>
                    <tfoot>
                        <tr>
                            <th colspan="3" class="text-right"><?php echo lang('grand_total'); ?>:</th>
...
jqry(document).ready(function() {
    ...
    // Sort by column 3 (Waste Qty) descending by default
    table.order([[3, 'desc']]);
